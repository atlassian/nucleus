...
import UploadBuildModal from './UploadBuildModal';
...
export default class ChannelVersionList extends React.PureComponent<
  ChannelVersionListProps,
  ChannelVersionListState
> {
  state = {
    loading: true,
  ...
  componentDidUpdate(prevProps: ChannelVersionListProps) {
    if (
      prevProps.app.id !== this.props.app.id ||
      prevProps.channel.id !== this.props.channel.id
    ) {
      this.fetch();
    }
    if (this.state.modalVersion && !this.state.modalVersion.isPreRelease) {
      const foundVersion = this.props.channel.versions.find(
        version => version.name === this.state.modalVersion.version.name
      );
      if (!foundVersion) return;
      if (
        JSON.stringify(this.state.modalVersion.version) !==
        JSON.stringify(foundVersion)
      ) {
        this.setState({
  ...
  showDraftVersionModal = (draftVersion: ITemporarySave) => {
    return this.showVersionModal(
      this.remapDraft(draftVersion),
      true,
      draftVersion.id,
      draftVersion.date
    );
            ...
            {this.props.channel.versions.length === 0 ? (
              ...
              this.props.channel.versions.map((version, index) => (
                <div
                  ...
                  onClick={this.showVersionModal(version)}
                  style={{
                    color: version.dead ? 'red' : 'inherit',
                    textDecoration: version.dead ? 'line-through' : ''
                  }}
                >
                  {version.name}
                  <span className={styles.versionRolloutSuper}>
                    {version.rollout}%
                  </span>
                </div>
              ))
            ...
            {this.state.temporaryVersions.length === 0 ? (
              ...
              this.state.temporaryVersions.map((version, index) => (
                <div
                  key={index}
                  className={styles.versionSelect}
                  onClick={this.showDraftVersionModal(version)}
                >
                  {version.version}
                </div>
              ))
            )}
            <AkButton appearance="primary" onClick={this.toggleUpload}>
              Upload New Build
            </AkButton>
            <UploadBuildModal
              onDismiss={this.toggleUpload}
              fetchVersions={this.fetch}
              isOpen={this.state.uploadingBuild}
              app={this.props.app}
  ...
  private toggleUpload = () => {
    this.setState({
      uploadingBuild: !this.state.uploadingBuild
    });
  };
  ...
  private download = file => () => {
    ...
    let downloadURL = `${this.props.baseUpdateUrl}/${this.props.app.slug}/${
      this.props.channel.id
    }/_index/${version}/${file.platform}/${file.arch}/${file.fileName}`;

    if (this.state.modalVersion && this.state.modalVersion.isPreRelease) {
      downloadURL = `/rest/app/${this.props.app.id}/channel/${
        this.props.channel.id
      }/temporary_releases/${this.state.modalVersion.preReleaseId}/${
        file.fileName
      }`;
    }
  ...
  private release = async () => {
    if (this.state.modalVersion && this.state.modalVersion.isPreRelease) {
      if (!confirm('Are you sure you want to release this pre-release?'))
        return;
      this.setState({
        actionRunning: true
      });
      const response = await fetch(
        `/rest/app/${this.props.app.id}/channel/${
          this.props.channel.id
        }/temporary_releases/${this.state.modalVersion.preReleaseId}/release`,
        {
          credentials: 'include',
          method: 'POST'
        }
      ...
      if (response.status === 409) {
        alert(
          'A release is already in progress, please wait a while and try again'
        );
        this.setState({
          actionRunning: false
        });
  ...
  private delete = async () => {
    if (this.state.modalVersion && this.state.modalVersion.isPreRelease) {
      ...
      const response = await fetch(
        `/rest/app/${this.props.app.id}/channel/${
          this.props.channel.id
        }/temporary_releases/${this.state.modalVersion.preReleaseId}/delete`,
        {
          credentials: 'include',
          method: 'POST'
        }
  ...
  private toggleVersionDeath = async () => {
    if (this.state.modalVersion && !this.state.modalVersion.isPreRelease) {
        ...
        !confirm(
          `Are you sure you want to ${
            this.state.modalVersion.version.dead
              ? 'bring this version back to life?'
              : 'mark this version as dead?'
          }`
        )
      )
        return;
      ...
      const response = await fetch(
        `/rest/app/${this.props.app.id}/channel/${this.props.channel.id}/dead`,
        {
          headers,
          credentials: 'include',
          method: 'POST',
  ...
  private modifyRollout = async () => {
    if (this.state.modalVersion && !this.state.modalVersion.isPreRelease) {
      const rawValue = prompt(
        'Please enter a new rollout percentage (between 0 and 100)'
      );
      if (typeof rawValue !== 'string') return;
      const newRollout = parseInt(rawValue, 10);
      if (isNaN(newRollout) || newRollout < 0 || newRollout > 100)
        return alert(
          'Invalid rollout percentage entered, expected a number between 0 and 100'
        );
      this.setState({
        actionRunning: true
      });
  ...
  render() {
    ...
    this.props.channel.versions.forEach((version, index) => {
      if (
        ...
        version.name === this.state.modalVersion.version.name
      ) {
        foundIndex = index;
      }
    });
    const hasFullRolloutAfter = this.state.modalVersion
      ? !!this.props.channel.versions.find(version => {
          return (
            !version.dead &&
            version.rollout === 100 &&
            semver.gt(version.name, this.state.modalVersion.version.name)
          );
        })
      : false;
    return (
      <div style={{ width: '100%' }}>
        <AkTabs tabs={this.getTabs()} />
        {this.state.modalVersion ? (
          <AkModalDialog
            header={
            ...
            footer={
              <div style={{ textAlign: 'right' }}>
                {!this.state.modalVersion.isPreRelease && hasFullRolloutAfter
                  ? this.killButton
                  : null}
                ...
                <AkButton
                  ...
                  isDisabled={this.state.actionRunning}
                >
                  Done
                </AkButton>
                {this.state.modalVersion.isPreRelease ? (
                ...
                {this.state.modalVersion.isPreRelease ? (
                  <AkButton
                    ...
                    isDisabled={
                      this.state.actionRunning || this.props.hasPendingMigration
                    }
                  >
                    Release
                  </AkButton>
          ...
          >
            <div
              className={`${styles.versionModalSpinner} ${
                this.state.actionRunning ? styles.versionModalSpinning : ''
              }`}
            >
              <AkSpinner size={40} />
            </div>
            {this.state.modalVersion.isPreRelease ? (
              <AkBanner icon={<WarningIcon label="Warning" />} isOpen>
                This is a set of pre-release files, click Release to make them
                public
              </AkBanner>
            ) : null}
            {this.state.modalVersion.version.dead ? (
            ...
            {this.state.modalVersion.isPreRelease ? (
              <p>Uploaded: {this.state.modalVersion.preReleaseDate}</p>
            ) : null}
            {this.state.modalVersion.isPreRelease ? null : (
              <div style={{ marginTop: 4 }}>
                <b>Current Rollout: </b>
                {this.state.modalVersion.version.rollout}%
                {this.state.modalVersion.version.rollout !== 100 &&
                !this.state.modalVersion.version.dead ? (
                  <a
                    href="javascript: void"
                    style={{
                      marginLeft: 8,
                      cursor: this.props.hasPendingMigration
                        ? 'not-allowed'
                        : 'pointer'
                    }}
                    onClick={
                      this.props.hasPendingMigration
                        ? () => {}
                        : this.modifyRollout
                    }
                  >
            ...
            {this.state.modalVersion.version.files.map((file, index) => (
              <div
                key={index}
                className={styles.fileDownloadContainer}
